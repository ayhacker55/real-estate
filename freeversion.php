...
// Fetch plan
$query = $db->query("SELECT `username`, `name`, `start_date`, `end_date` FROM `plans` WHERE `username` ='$username'");
if ($query && $query->num_rows === 1) {
    $plan = $query->fetch_assoc();
    $start = $plan['start_date'];
    $end   = $plan['end_date'];

    // If start date equals end date, redirect
    if ($start == $end) {
        header('Location: expired.php');
        exit;
    } 

    // If today is before end date, calculate remaining days
    $today = new DateTime();
    ...
    if ($today < $endDate) {
        ...
        // Update days column in database
        $update = $db->prepare("UPDATE `plans` SET `days` = ? WHERE `username` = ?");
        $update->bind_param("is", $daysRemaining, $plan['username']);
        $update->execute();
    } else {
        // End date passed
        header('Location: expired.php');
...
<div id="add" class="tab">
  <div id="add-default-message">Please select a form</div>
  <div id="land-sale" class="form-container" style="display: none;"></div>
  <div id="land-rent" class="form-container" style="display: none;"></div>
  <div id="property-sale" class="form-container" style="display: none;"></div>
  <div id="property-rent" class="form-container" style="display: none;"></div>
  <div id="shortlet" class="form-container" style="display: none;"></div>
  <div id="decor" class="form-container" style="display: none;"></div>
  <div id="building-material" class="form-container" style="display: none;"></div>
</div>
...
<div id="active" class="status-list" style="display: none;"></div>
<div id="pending" class="pending-wrapper" style="display: none;"></div>
   ...
   <!-- approval -->
    ...
    <div id="subscription" class="tab">
      <h1>free Trial versiom Details</h1>
      <div class="card-grid">
        <div class="card">
          <h3>Plan Type</h3>
          <p>free trial</p>
...
<div class="dashboard">
  <div id="sidebar" class="sidebar">
    ...
    <div class="menu-item">
      ...
      <button class="submenu-btn add-btn" data-form="land-sale">Land for Sale</button>

        <button class="submenu-btn" data-form="land-rent">Land for Rent</button>
        <button class="submenu-btn" data-form="property-sale">Property for Sale</button>
        <button class="submenu-btn" data-form="property-rent">Property for Rent</button>
        <button class="submenu-btn" data-form="shortlet">Shortlet</button>
        <button class="submenu-btn" data-form="decor">Decor</button>
        <button class="submenu-btn" data-form="building-material">Building Material</button>
      </div>
    </div>
  ...
  <button class="tab-btn" data-tab="view">🔍 View Property</button>
  <div class="submenu" style="display: none;">
 <button class="submenu-btn view-btn" data-form="active">Active</button>
<button class="submenu-btn view-btn" data-form="pending">Pending</button>
